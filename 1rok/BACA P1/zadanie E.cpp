...
struct Sklad {
    ...
    Regal podregal;
    Polka podpolka;
    unsigned short silosc;} static st;
...
void setas(unsigned short w, unsigned short r, unsigned short s, unsigned short p, Regal& regal) {
    for (int i = s; i < regal.pilosc; i++) {
        ...
        for (int n = 0; n < 128; n++) suma -= regal.polka[i].miejsce[n].towar;}
...
void putw(unsigned short w, unsigned short r, unsigned short s, unsigned short p, unsigned short A) {
    if (w >= st.silosc || r >= st.magazyn[w].rilosc || s >= st.magazyn[w].regal[r].pilosc || p >= st.magazyn[w].regal[r].polka[s].milosc) cout << "error" << endl;
    else {
...
void puth(unsigned short w, unsigned short p, unsigned short A) {
    if (w >= st.silosc || p >= st.magazyn[w].handyshelf.milosc) cout << "error" << endl;
    else {
...
void putr(unsigned short s, unsigned short p, unsigned short A) {
    if (s >= st.podregal.pilosc || p >= st.podregal.polka[s].milosc) cout << "error" << endl;
    else {
...
void puts(unsigned short p, unsigned short A) {
    if (p >= st.podpolka.milosc) cout << "error" << endl;
    else {
...
int main(){
    ...
    while (operacja[0] != 'E' && operacja[1] != 'N' && operacja[2] != 'D') {
        ...
        if (operacja[0] == 'S' && operacja[1] == 'E' && operacja[2] == 'T' && operacja[3] == '-' && operacja[4] == 'A' && operacja[5] == 'P') {
            cin >> w >> r >> s >> p;
            if (w >= st.silosc || r >= st.magazyn[w].rilosc || s >= st.magazyn[w].regal[r].pilosc || p > 128) cout << "error" << endl;
            else setap(w, r, s, p, st.magazyn[w].regal[r].polka[s]);}
            
        else if (operacja[0] == 'S' && operacja[1] == 'E' && operacja[2] == 'T' && operacja[3] == '-' && operacja[4] == 'A' && operacja[5] == 'S') {
            cin >> w >> r >> s >> p;
            if (w >= st.silosc || r >= st.magazyn[w].rilosc || s > 128 || p > 128) cout << "error" << endl;
            else setas(w, r, s, p, st.magazyn[w].regal[r]);}
            
        else if (operacja[0] == 'S' && operacja[1] == 'E' && operacja[2] == 'T' && operacja[3] == '-' && operacja[4] == 'A' && operacja[5] == 'R') {
            cin >> w >> r >> s >> p;
            if (w >= st.silosc || r > 128 || s > 128 || p > 128) cout << "error" << endl;
            else setar(w, r, s, p, st.magazyn[w]);}
            ...
            if (w >= st.silosc || r >= st.magazyn[w].rilosc || s >= st.magazyn[w].regal[r].pilosc || p >= st.magazyn[w].regal[r].polka[s].milosc || ww >= st.silosc || rr >= st.magazyn[w].rilosc || ss >= st.magazyn[w].regal[r].pilosc) cout << "error" << endl;
            else {
                for (int i = 0; i < A; i++) {
                    if (st.magazyn[w].regal[r].polka[s].miejsce[p].towar - 1 < 0 || 65535 < st.magazyn[ww].regal[rr].polka[ss].miejsce[p].towar + 1) break;
                    else {
            ...
            cin >> w >> r >> s >> p >> A;
            if (w >= st.silosc || r >= st.magazyn[w].rilosc || s >= st.magazyn[w].regal[r].pilosc || p >= st.magazyn[w].regal[r].polka[s].milosc || p >= st.magazyn[w].handyshelf.milosc) cout << "error" << endl;
            else {
                for (int i = 0; i < A; i++) {
                    if (st.magazyn[w].regal[r].polka[s].miejsce[p].towar - 1 < 0 || 65535 < st.magazyn[w].handyshelf.miejsce[p].towar + 1) break;
                    else {
                        st.magazyn[w].regal[r].polka[s].miejsce[p].towar -= 1;
            ...
            cin >> w >> r >> s >> ss >> p >> A;
            if (w >= st.silosc || r >= st.magazyn[w].rilosc || s >= st.magazyn[w].regal[r].pilosc || p >= st.magazyn[w].regal[r].polka[s].milosc || ss >= st.podregal.pilosc || p >= st.podregal.polka[ss].milosc) cout << "error" << endl;
            else {
                for (int i = 0; i < A; i++) {
                    if (st.magazyn[w].regal[r].polka[s].miejsce[p].towar - 1 < 0 || 65535 < st.podregal.polka[ss].miejsce[p].towar + 1) break;
                    else {
                        st.magazyn[w].regal[r].polka[s].miejsce[p].towar -= 1;
            ...
            cin >> s >> p >> A;
            if (s >= st.podregal.pilosc || p >= st.podregal.polka[s].milosc || p >= st.podpolka.milosc) cout << "error" << endl;
            else {
                for (int i = 0; i < A; i++) {
                    if (st.podregal.polka[s].miejsce[p].towar - 1 < 0 ||  65535 < st.podpolka.miejsce[p].towar + 1) break;
                    else {
            ...
            cin >> w >> r >> s;
            if (w >= st.silosc || r >= st.magazyn[w].rilosc || s >= st.magazyn[w].regal[r].pilosc) cout << "error" << endl;
            else getsw(w, r, s);}
            ...
            cin >> s;
            if (s >= st.podregal.pilosc) cout << "error" << endl;
            else getsr(s);}
            ...
            if (w >= st.silosc || p >= st.podpolka.milosc) cout << "error" << endl;
                ...
                if (st.magazyn[w].handyshelf.miejsce[p].e[0] == '0' && st.magazyn[w].handyshelf.miejsce[p].e[1] == '0') cout << "--" << endl;
                else getlh(w, p);}
        }
            ...
            if (s >= st.podregal.pilosc || p >= st.podregal.polka[s].milosc) cout << "error" << endl;
                ...
                if (st.podregal.polka[s].miejsce[p].e[0] == '0' && st.podregal.polka[s].miejsce[p].e[1] == '0') cout << "--" << endl;
                else getlr(s, p);}
        }
            ...
            if (p >= st.podpolka.milosc) cout << "error" << endl;
                ...
                if (st.podpolka.miejsce[p].e[0] == '0' && st.podpolka.miejsce[p].e[1] == '0') cout << "--" << endl;
                else getls(p);}
        }
    }
}
