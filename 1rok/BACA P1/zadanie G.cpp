...
struct Lines {string a1, a2, a3, a4, a5, a6, a7;};
...
Str tworzenie2(string file1, int numerlin) {
    ...
    string temp;
    for (int i = 0; i < numerlin; i++) {getline(f1, temp);}
    Str str;
    f1 >> str.liczba;
...
void kopia(string file1, string file2) {
    ...
    while (getline(f1, line)) {f2 << line << endl;}
    f1.close();
    f2.close();}
...
void app1(string file2, Str str) {
    ...
    f2 << str.liczba << endl << str.tekst << endl << str.letter << endl << str.l1 << ' ' << str.s1 << ' ' << str.d1 << endl << str.l2 << ' ' << str.s2 << ' ' << str.d2 << endl << str.l3 << ' ' << str.s3 << ' ' << str.d3 << endl << str.l4 << ' ' << str.s4 << ' ' << str.d4 << endl;
    f2.close();}
...
void SortInt(string file1, string file2, string file3) {
    ...
    if (n != 0) {
        ...
        for (int i = 0; i < n - 1; i++){
            if (i % 2 == 0) {
                ...
                for (int j = 0; j < n - i - 1; j++) {
                    if (!stan) {str1 = tworzenie2(file2, j * 7);}
                    else {stan = 0;}
                    ...
                    if (str1.liczba > str2.liczba) {
                        app1(file3, str2);
                        stan = 1;}
                    else {app1(file3, str1);}}
                    
                if (!stan) {app1(file3, str2);}
                else {app1(file3, str1);}
                ...
                int stan = 0;
                for (int j = 0; j < n - i - 1; j++) {
                    if (!stan) {str1 = tworzenie2(file3, j * 7);}
                    else {stan = 0;}
                    ...
                    if (str1.liczba > str2.liczba) {
                        app1(file2, str2);
                        stan = 1;}
                    else {app1(file2, str1);}}
                    
                if (!stan) {app1(file2, str2);}
                else {app1(file2, str1);}
        ...
        if (n % 2 == 0) {kopia(file3, file1);}
        else {kopia(file2, file1);}}}
...
void app2(string file2, Lines lin) {
    fstream f2;
    f2.open(file2.c_str(), fstream::app | fstream::out);
    f2 << lin.a1 << endl<< lin.a2 << endl<< lin.a3 << endl<< lin.a4 << endl<< lin.a5 << endl<< lin.a6 << endl<< lin.a7 << endl;
    f2.close();}
...
void usun(string file2, string file3, int numerlin) {
    ...
    for (int i = 0; i < 7; i++) {getline(f3, line);}
    
    while (getline(f3, line)) {f2 << line << endl;}
    f2.close();
    f3.close();}
