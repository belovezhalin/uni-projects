...
string AddSubInSum(string number1, string number2) {
    ...
    if(number1 != "0" && StringToIntInSum(number1) != 0) DeleteZeroInSum(number1);
    if(number2 != "0" && StringToIntInSum(number2) != 0) DeleteZeroInSum(number2);

    if(number1 == "" && number2 == "") return "";
    else if(number1 == "" && number2 != "") {
        if(minus2) return "-" + number2;
        else return number2;}
    else if(number1 != "" && number2 == "") {
        if(minus1) return "-" + number1;
        else return number1;}

    if(!minus1 && !minus2) result = AddInSum(number1, number2);

    else if(minus1 && minus2) {
        result = AddInSum(number1, number2);
        if(result != "0") DeleteZeroInSum(result);
        result = '-' + result;}
