...
int main() {
    ...
    *rows = 0;
    ...
    while(strcmp(operation, "END") != 0) {
        ...
        else if(strcmp(operation, "DFR") == 0) {DFR(&tab, rows, &columns);}
        
		else if(strcmp(operation, "DLR") == 0) {DLR(&tab, rows, &columns);}

        else if(strcmp(operation, "DFC") == 0) {DFC(&tab, rows, &columns);}
            
        else if(strcmp(operation, "DLC") == 0) {DLC(&tab, rows, &columns);}
		...
		else if(strcmp(operation, "WRF") == 0) {WRF(&tab, rows, &columns);}

		else if(strcmp(operation, "RDF") == 0) {RDF(&tab, rows, &columns);}

        else if(strcmp(operation, "PRT") == 0) {PRT(tab, rows, columns);}
        
        fflush(stdin);
        scanf("%s", operation);}

    while(*rows != 0) {DFR(&tab, rows, &columns);}
