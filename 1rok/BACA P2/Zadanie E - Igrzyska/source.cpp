...
public:
    ...
    void takeDamage(unsigned int damage) {
        if(damage > defense + agility) {
            if((int)(currentHP - (damage - (defense + agility))) <= 0) this->die();
            else currentHP -= (damage - (defense + agility));
        }
    }
...
public:
    ...
    void takeDamage(unsigned int damage) {
        if(damage > 0.5 * agility) {
            if((int)(currentHP - (damage - (0.5 * agility))) <= 0) this->die();
            else currentHP -= (damage - (0.5 * agility));
        }
    }
...
public:
    ...
    string getID() {
        if(this->getRemainingHealth() >= 25 || this->getRemainingHealth() == 0) return HUMAN_CLASS::id;
        else return BEAST_CLASS::id;
    }
    ...
    unsigned int getAgility() {
        if(this->getRemainingHealth() >= 25 || this->getRemainingHealth() == 0) return HUMAN_CLASS::getAgility();
        else return BEAST_CLASS::getAgility();
    }

    unsigned int getDamage() {
        if(this->getRemainingHealth() >= 25 || this->getRemainingHealth() == 0) return HUMAN_CLASS::getDamage();
        else return BEAST_CLASS::getDamage();
    }

    void takeDamage(unsigned int damage) {
        if(this->getRemainingHealth() >= 25 || this->getRemainingHealth() == 0) HUMAN_CLASS::takeDamage(damage);
        else BEAST_CLASS::takeDamage(damage);
    }

    void printParams() {
        if(this->getRemainingHealth() >= 25 || this->getRemainingHealth() == 0) HUMAN_CLASS::printParams();
...
public:
    ...
    unsigned int getDefense() {
        ...
        if(countPlayers() != 0) {
            ...
            while(pointer != NULL) {
                if(pointer->getRemainingHealth() != 0) counterDefense += pointer->getDefense();
                pointer = pointer->next;
            }
    ...
    unsigned int getAgility() {
        ...
        if(countPlayers() != 0) {
            ...
            if(pointer->getRemainingHealth() != 0) counterAgility = pointer->getAgility();
            while(pointer != NULL) {
                if(pointer->getRemainingHealth() != 0) {
                    if(counterAgility > pointer->getAgility()) counterAgility = pointer->getAgility();
                }
                pointer = pointer->next;
            }
    ...
    void applyWinnerReward() {
        ...
        if(countPlayers() != 0) {
            while(pointer != NULL) {
                if(pointer->getRemainingHealth() != 0) pointer->applyWinnerReward();
                pointer = pointer->next;
            }
        }
    }
    ...
    unsigned int getDamage() {
        ...
        if(countPlayers() != 0) {
            int damageCounter = 0;
            while(pointer != NULL) {
                if(pointer->getRemainingHealth() != 0) damageCounter += pointer->getDamage();
                pointer = pointer->next;
            }
    ...
    void printParams() {
        if(countPlayers() != 0) {
            ...
            PLAYER_CLASS *pointer = first;
            while(pointer != NULL) {
                if(pointer->getRemainingHealth() != 0) pointer->printParams();
                pointer = pointer->next;
            }
        } 
        else {
    ...
    void cure() {
        ...
        if(countPlayers() != 0) {
            while(pointer != NULL) {
                if(pointer->getRemainingHealth() != 0) pointer->currentHP = pointer->maxHP;
                if(pointer->getRemainingHealth() > getRemainingHealth()) {
                    currentHP = pointer->currentHP;
                    maxHP = pointer->maxHP;
                }
