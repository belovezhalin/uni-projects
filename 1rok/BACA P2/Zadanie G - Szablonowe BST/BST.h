...
public:
    ...
    void PrintLEVELORDER(Node *node, int i)
    {
        if (node)
        {
            if (i == 1) cout << node->key << node->data;
            else if (1 < i)
            {
    ...
    DATA_TYPE* Search(Node* node, KEY_TYPE keyinput)                                   ////////////////////////?? tylko operator <
    {
        if (node)
        {
            if (!(node->key < keyinput) && !(keyinput < node->key)) return &(node->data);
            else if (keyinput < node->key) return Search(node->left, keyinput);
            else return Search(node->right, keyinput);
        }
        return NULL;
    }
    ...
    Node* Delete(Node* node, KEY_TYPE keyinput, bool& isdeleted)
    {
        ...
        {    
            ...
            if (!(!(node < nextparent) && !(nextparent < node))) nextparent->left = mynext->right;
            else nextparent->right = mynext->right;
