...
int podziel(const char *file) {
    ...
    int output1 = open("fragment1", O_WRONLY | O_CREAT | O_TRUNC);
    int output2 = open("fragment2", O_WRONLY | O_CREAT | O_TRUNC);

    if(input == -1) return -1;
    if(output1 == -1) return -1;
    if(output2 == -1) return -1;

    chmod("fragment1", S_IRUSR | S_IWUSR);
    chmod("fragment2", S_IRUSR | S_IWUSR);
