...
string FormatujNapis(string napis, string arg1, string arg2, string arg3)
{
	...
	for (int i = 0; i < napis.size(); i++)
	{
		if (napis[i] == '{')
		{
			...
			case 'U':
				if ((int)zwrot.size() - napis[i + 3] + '0' < 0)
					zwrot = "";
				else
					zwrot.resize(zwrot.size() - napis[i + 3] + '0');
				i += 4;
				break;
			...
			case 'W':
				...
				{
					for (int a = argument.size(); a < napis[i + 5] - '0'; a++)
						argument += ' ';
				}
				else if (napis[i + 5] - '0' < argument.size())
					argument.resize(napis[i + 5] - '0');
...
string UsunSlowo(string napis, int liczba)
{
	...
	for (int i = 0; i < napis.size(); i++)
	{
		if (napis[i] == ' ' && slowo == true)
			numer++;

		slowo = napis[i] != ' ';

		if (!slowo || liczba != numer)
			zwrot += napis[i];
	}
