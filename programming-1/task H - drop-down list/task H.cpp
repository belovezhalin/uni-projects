		...
		if ((*q) != wsk)
		{
			...
			if (bi != 0)
			{
				...
				*(wsk->object + bi - 1) = *wezel;
				...
				Function(*array, *q, (*q)->use - 1, wezel);
			*array = b;
		}
...
void RemoveFirst(NODE_STRUCT **array)
{
	...
	if ((*array)->use == 0)
	{
		...
		for (int i = 0; i < (*array)->use; *((*array)->object + i) = *((*array)->object + i + 1), i++)
			;
}
/////////////////////////////////////////////////////////////////////////////////////////
void RemovePrev(NODE_STRUCT **array, NODE_STRUCT *wsk, BYTE i)
{
	if (i > 0)
	{
		...
		if (wsk->use == 0)
		{
			...
			for (int j = i - 1; j < wsk->use; *(wsk->object + j) = *(wsk->object + j + 1), j++)
				;
	}
	else
	{
		...
		if (b->use == 0)
		{
			...
			for (int j = i; j < b->use; *(b->object + j) = *(b->object + j + 1), j++)
				;
	}
}
/////////////////////////////////////////////////////////////////////////////////////////
void RemoveCurrent(NODE_STRUCT **array, NODE_STRUCT *wezel, BYTE i)
{
	...
	if (wezel->use == 0)
	{
		...
		for (int j = i; j < wezel->use; *(wezel->object + j) = *(wezel->object + j + 1), j++)
			;
}
/////////////////////////////////////////////////////////////////////////////////////////
